 ...
 * Returns results in stable order: k8s, prometheus, mediamtx, kafka, s3.
 * Never throws.
 */
export async function collectConnectivity(): Promise<BackendStatus[]> {
  const [k8s, prometheus, mediamtx, kafka, s3] = await Promise.all([
  ...
  return [k8s, prometheus, mediamtx, kafka, s3];
}
