...
let createScene, PALETTE;
try {
  ({ createScene, PALETTE } = await import(LIB));
} catch {
...
S.rect('z_nav', 40, 92, 1820, 300, { bg: P.zone.blue, stroke: P.blue, sw: 1, op: 30 });
...
legend.forEach(([bg, stroke, label], i) => {
  S.rect(`lg_${i}`, 60 + i * 470, 352, 450, 28, { bg, stroke, fs: 13, label });
});

// ── band 2: the state shared with the deployer ────────────────────────────────
S.rect('z_state', 40, 412, 900, 300, { bg: P.zone.purple, stroke: P.purple, sw: 1, op: 28 });
S.text('t_state', 60, 422, 'State shared with the deployer', 18, P.ink.purple);
S.text('t_statec', 60, 446,
  'the deployer provisions the cluster, then both write the same Firestore documents', 14, P.muted);

S.rect('st_dep', 62, 478, 250, 60, {
  bg: P.fill.amber, stroke: P.amber, fs: 14, label: 'deployer :5002\nlaptop-side, pre-install',
});
S.rect('st_con', 668, 478, 250, 60, {
  bg: P.fill.blue, stroke: P.blue, fs: 14, label: 'console :8800\nin-cluster, post-install',
});
S.rect('st_fs', 344, 470, 292, 76, {
...
S.arrow('a_dep', 316, 508, [[0, 0], [24, 0]], { stroke: P.amber });
S.arrow('a_con', 640, 508, [[24, 0], [0, 0]], { stroke: P.blue });

m.configStore.paths.forEach((p, i) => {
  S.rect(`fs_${i}`, 62 + (i % 2) * 428, 566 + Math.floor(i / 2) * 38, 418, 32, {
    bg: '#ffffff', stroke: P.purple, sw: 1, fs: 13, label: p,
  });
});
S.text('t_reconcile', 62, 648, wrapToWidth(
...
S.rect('z_reach', 960, 412, 900, 300, { bg: P.zone.green, stroke: P.green, sw: 1, op: 25 });
S.text('t_reach', 980, 422, 'What the API routes reach', 18, P.ink.green);
S.text('t_reachc', 980, 446,
...
S.rect('z_find', 40, 732, 1820, 216, { bg: P.zone.amber, stroke: P.amber, sw: 1, op: 25 });
S.text('t_find', 60, 742, 'What the source says', 18, P.ink.amber);
...
findings.forEach(([head, body], i) => {
  const x = 62 + i * 600;
  S.rect(`f_${i}`, x, 774, 576, 158, { bg: '#ffffff', stroke: P.amber, sw: 1 });
  S.text(`fh_${i}`, x + 16, 786, head, 15, '#b45309');
  S.text(`fb_${i}`, x + 16, 812, wrapToWidth(body, 62), 13, '#3d4a56');
});
