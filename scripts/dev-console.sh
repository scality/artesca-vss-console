...
say "Writing $ENVF"
{
  ...
  # Auth.js initializes its /api/auth/session route even with auth disabled, and aborts
  # with MissingSecret if no secret is set. SessionProvider polls that route, so without
  # this it 500s and surfaces a ClientFetchError dev overlay. A dev-only secret silences
  # it; deployed pods get NEXTAUTH_SECRET from the console-auth Secret. Regenerated per run.
  DEV_AUTH_SECRET="$(openssl rand -base64 32)"
  echo "AUTH_SECRET=$DEV_AUTH_SECRET"
  echo "NEXTAUTH_SECRET=$DEV_AUTH_SECRET"
