...
| `/capabilities` | The agent's tool catalog (hand-curated, not a live introspection) with a live reachability chip, plus a pointer card to `/agent` for the routing config (system prompt, reasoning budget, LLM wiring) — no duplicated config values on this page. |
