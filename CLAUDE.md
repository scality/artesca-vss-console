...
18 pages, all server components by default; client components are scoped to interactive bits (forms, auto-refresh).
...
| `/capabilities` | The agent's tool catalog (hand-curated, not a live introspection) with a live reachability chip and a read-only **Agent behavior** card — the live `vss-agent-config` system prompt, `max_iterations`, and LLM name/base URL — linking to `/agent` for editing. |
| `/agent` | View **and edit** the live vss-agent routing config: system prompt, `max_iterations` (reasoning budget), and LLM name/base URL, with a live health badge (`● reachable` / `✕ URL misconfigured` / `✕ auth failed` / `▲ unreachable`) from a `{LLM_BASE_URL}/v1/models` probe. **Save + Restart** writes via `PATCH /api/agent-config` — patches the `vss-agent-config` ConfigMap (prompt / `max_iterations`) and the `vss-agent` Deployment env (`LLM_BASE_URL` / `LLM_NAME`), then rolls the Deployment. Warns without blocking on a trailing `/v1` in the base URL — the agent appends `/v1` itself, doubling to `/v1/v1` and 404ing every query. |
