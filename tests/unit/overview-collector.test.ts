...
describe("collectOverviewSnapshot — degraded-snapshot contract (k8s mode)", () => {
  ...
  it("Kafka admin throws → returns result with Kafka warning, topics get zero lag", async () => {
    const failingAdmin = {
    ...
    // Topics should still be present with zero lag.
    for (const entry of Object.values(result.snapshot.kafka)) {
      expect(entry.consumerLagMsgs).toBe(0);
    }
  });
