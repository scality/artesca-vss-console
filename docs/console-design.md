...
| F | **SG whitelist: owned by the lab tooling, not the console.** `scality/isv-labs/scripts/providers/aws/seed-sg-whitelist.sh` writes the EC2 ingress rules across every admin port. | The console has no AWS credentials and no EC2 permissions. It held a `/settings` → "Network access" panel over a `console-aws` Secret until ISVD-610: that panel read an env var no deploy ever set, so it could not write on any deployment, while the script above already owned the writes and described the console's copy as "a display mirror". A customer's ARTESCA cluster has no EC2 for such a panel to act on. |
