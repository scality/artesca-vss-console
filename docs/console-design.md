...
| # | Decision | Impact |
| --- | --- | --- |
| 1 | **Access**: Scality engineers from their laptops via SG-whitelisted NodePort. Cluster-internal HTTP. No public TLS in scope. | No cert-manager / ingress rewrite. SG allows `:8800` from a curated IP list (Scality office + VPN + per-engineer home IPs). |
...
| `/incidents` | [kiosk] | Live feed (replaces the standalone alert dashboard for console users). Filter by scenario / sensor / severity / time-window. Click an incident → **play the source clip** (HLS via `hls.js`, server-side ffmpeg proxy from ARTESCA S3) + raw Kafka payload + thumbnail. **Generate Report**: a per-incident button in the detail dialog asks the live agent LLM to synthesize a structured markdown incident report (Overview / Location / Involved parties & vehicles / Timeline / Evidence) from the VLM reasoning + metadata + extracted clip frames — the Evidence section (frame stills + clip link) is appended deterministically and an LLM failure degrades to a deterministic fallback; reports persist in SQLite (`incident_reports`, keyed `{sensorId, ts}`) and re-open with a Regenerate option. Mirrors the NVIDIA VSS Warehouse blueprint's per-alert report action. |
...
| `/settings` | — | Console-level config: **Network access** sub-panel — CIDR allow-list for `:8800` with add/remove (writes to the EC2 SG via `console-aws` creds + audit log). Lab-only: the panel and its routes are present only where `VSS_INSTANCE_SG_ID` and `AWS_REGION` are both set, and are absent on a cluster with no EC2 in front of it. Kiosk-mode toggle persistence, feature flags, SSH key rotation for camera-sim, inspect current ServiceAccount permissions. |
...
| POST | `/api/incidents/report` | Generate a structured markdown incident report (agent LLM over VLM reasoning + metadata + clip frames); fail-soft to a deterministic report; persists to SQLite `incident_reports`. `GET` with `?sensorId=&ts=` returns the cached report |
