...
| `deploy-console.sh` exits with "could not fetch config-store-rw-key" | `gcloud` ADC stale or SA key missing from Secret Manager | `gcloud auth login --update-adc`, then confirm `gcloud secrets versions access latest --secret=config-store-rw-key --project=isv-alliances` returns non-empty output. |
